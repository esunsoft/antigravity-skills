...
description: Analyze codebase architecture, understand components and relationships. Use when exploring unfamiliar code or documenting system structure
argument-hint: [diff|explain|onboard]
---
...
2. Detect whether git is available and whether `PROJECT_ROOT` is inside a git repository.
   - If yes, get the current git commit hash:
     ```powershell
     git rev-parse HEAD
     ```
   - If no, set the current git commit hash to `null` and continue in no-git mode.
3. Create the intermediate output directory:
   ```powershell
   New-Item -ItemType Directory -Path "$PROJECT_ROOT\.understand-anything\intermediate" -Force
   ```
4. Check if `$PROJECT_ROOT/.understand-anything/knowledge-graph.json` exists. If it does, read it.
5. Check if `$PROJECT_ROOT/.understand-anything/meta.json` exists. If it does, read it to get `gitCommitHash` and any previous analysis mode.
6. **Decision logic:**

   | Condition | Action |
   |---|---|
   | `--full` flag in `$ARGUMENTS` | Full analysis (all phases) |
   | git available + no existing graph or meta | Full analysis (all phases) |
   | git available + existing graph + unchanged commit hash | Report "Graph is up to date" and STOP |
   | git available + existing graph + changed files | Incremental update (re-analyze changed files only) |
   | git unavailable | Full analysis (all phases) |

   If git is available and incremental updates are possible, get the changed file list:
   ```powershell
   ...
   - Do not attempt incremental updates
   - Run a full rebuild instead
   - If `$ARGUMENTS` contains a directory path, use it as the analysis scope to reduce work
   - Report clearly that git was not detected and the run is using `no-git-full` mode
...
7. **Collect project context for subagent injection:**
   ...
   - Capture the top-level directory tree:
     ```powershell
     Get-ChildItem -Path $PROJECT_ROOT -Recurse -Depth 2 -File | Where-Object { $_.FullName -notmatch 'node_modules|\.git|dist' } | Select-Object -First 100 -ExpandProperty FullName
     ```
     Store as `$DIR_TREE`.
...
```json
{
  ...
  "project": {
    ...
    "gitCommitHash": "<commit hash from Phase 0 or null when git is unavailable>"
  },
...
2. Write metadata to `$PROJECT_ROOT/.understand-anything/meta.json`:
   ```json
   {
     ...
     "gitCommitHash": "<commit hash or null>",
     "analysisMode": "<git-incremental | git-full | no-git-full>",
     "version": "1.0.0",
...
3. Clean up intermediate files:
   ```powershell
   Remove-Item -Path "$PROJECT_ROOT\.understand-anything\intermediate" -Recurse -Force
   ```

4. Report a summary to the user containing:
   - Project name and description
   - Analysis mode (`git-incremental`, `git-full`, or `no-git-full`)
   ...
   - If git was unavailable, explicitly note that a full rebuild was used instead of incremental analysis
