...
3. Find the dashboard code. The dashboard is at `packages/dashboard/` relative to this plugin's root directory. Resolve the path using PowerShell:
   ```powershell
   $PLUGIN_ROOT = Split-Path -Parent (Split-Path -Parent $PSScriptRoot)
   ```
...
4. Install dependencies and build if needed:
   ```powershell
   Set-Location <dashboard-dir>; pnpm install --frozen-lockfile 2>$null; if ($LASTEXITCODE -ne 0) { pnpm install }
   ```
   Then ensure the core package is built (the dashboard depends on it):
   ```powershell
   Set-Location <plugin-root>; pnpm --filter @understand-anything/core build
   ```

5. Start the Vite dev server pointing at the project's knowledge graph:
   ```powershell
   Set-Location <dashboard-dir>; $env:GRAPH_DIR = "<project-dir>"; npx vite --open
   ```
